...
Chinamap.prototype = {
    init : function(container, options)  {
        ...
        map.attr = {
            ...
            flashing : {
                ...
                "stroke" : map.options.point["border-color"],
                "stroke-width" : map.options.point["border-width"],
                "opacity" : map.options.point.opacity,
                "r" : 0
            },
            line : {
    ...
    flashing : function(id, loop) {
        ...
        map.el.flash[id].c1.animate({r: r * map.options.point["flash-scale"], opacity: map.options.point.opacity}, 2000, '', function() {
            map.el.flash[id].c1.animate({opacity: 0}, 1500, '', function() {
                map.el.flash[id].c1.attr(map.attr.flashing);
            });
            map.el.flash[id].c2.attr(map.attr.flashing);
            if (!loop) {
                map.el.point[id].attr({r: 0}).show().animate({r: r}, 1500, '', function(){
                    map.el.flash[id].do = false;
                });
            } else {
                map.el.flash[id].c2.animate({r: r * map.options.point["flash-scale"], opacity: map.options.point.opacity}, 2000, '', function() {
                    map.el.flash[id].c2.animate({opacity: 0}, 1500, '', function() {
                        map.el.flash[id].c2.attr(map.attr.flashing);
                    });
                    map.flashing(id, loop);
                });
