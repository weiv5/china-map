...
Chinamap.prototype = {
    init : function(container, options)  {
        ...
        map.paper.setViewBox(-1255, -1535, 3200, 2670, true);
        map.projection = d3.geo.albers().origin([105, 30]).scale(4000);
    ...
    point : function(data) {
        ...
        var r_plus = d3.scale.linear().domain([0, max]).range([0, map.options.point.radius]),
            radius = function(num) {return r_plus(num) + 8;}
        for (var i in tmp) {
            (function(id, num){
                var r = radius(num);
                if (map.el.point[id]) {
                    var onum = map.el.point[id].data("data");
                    map.el.point[id].animate({
                        "r" : r
                    }, 300).data({
                        "or": r,
                        "data" : num
