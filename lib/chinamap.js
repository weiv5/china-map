...
Chinamap.prototype = {
    ...
    line : function(id1, id2) {
        ...
        d3.timer(function(elapsed) {
            ...
            if (t>=1.01) {
                ...
                line.attr({path: path(p1, nextPo(p1, p2, t))});
            }
        });
    },
    flash : function(id, loop) {
        ...
        var loc = map.projection(data_point[id].loc);
        if (map.el.flash == null) {
            map.el.flash = {};
        }
        if (typeof map.el.flash[id] === 'undefined') {
            map.el.flash[id] = [];
            map.el.flash[id][0] = map.paper.circle(loc[0], loc[1], 0).attr({"fill": "orange"}),
            map.el.flash[id][1] = map.paper.circle(loc[0], loc[1], 0).attr({"fill": "orange"});
        }
        map.el.flash[id][0].animate({r: r * map.options.point["flash-scale"]}, 2000, '', function() {
            map.el.flash[id][0].animate({opacity: 0}, 1000, '', function() {
                map.el.flash[id][0].attr({opacity:map.options.point.opacity, r: 0});
            });
            map.el.flash[id][1].animate({r: r * map.options.point["flash-scale"]}, 2000, '', function() {
                map.el.flash[id][1].animate({opacity: 0}, 1000, '', function() {
                    map.el.flash[id][1].attr({opacity:map.options.point.opacity, r: 0});
                });
                if (loop) {
                    map.flash(id, loop);
                } else {
                    map.el.point[id].attr({r: 0}).show().animate({r: r}, 2000);
                }
            });
        });
    },
