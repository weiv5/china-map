...
Chinamap.prototype = {
    ...
    line : function(id1, id2) {
        ...
        d3.timer(function(elapsed) {
            ...
            if (t>=1.01) {
                ...
                map.flash(id2, false, map.options.line.color);
    ...
    flash : function(id, loop, color) {
        var map = this;
        if (typeof map.el.point[id] === 'undefined') {
            return;
        }
    ...
    flashColor : function(id, color) {
        ...
        map.el.flash[id][0].attr(attr),
        map.el.flash[id][1].attr(attr);
    },
    flashColorRecover : function(id, idx) {
        var map = this;
        if (typeof map.el.flash[id] === "undefined") {
            return;
        }
    ...
    flashing : function(id, loop) {
        ...
        map.el.flash[id][0].animate({r: r * map.options.point["flash-scale"], opacity: map.options.point.opacity}, 2000, '', function() {
            map.el.flash[id][0].animate({opacity: 0}, 1500, '', function() {
                ...
                map.flashColorRecover(id, 0);
            });
            map.flashColorRecover(id, 1);
            if (!loop) {
